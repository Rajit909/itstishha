...
const getStudiesForCategory = async (category: ProjectCategory): Promise<Project[]> => {
  const allStudies = await getProjects();
...
export default async function ProjectCategoryPage({ params }: ProjectCategoryPageProps) {
  ...
  const projects = await getStudiesForCategory(category);
          ...
          {projects.length > 0 ? (
            <div className="grid gap-8 md:grid-cols-1 lg:grid-cols-2">
             ...
             <div className="text-center py-12 border rounded-lg bg-card animate-fade-in-up">
              <h3 className="font-headline text-2xl font-semibold">No Projects Found</h3>
